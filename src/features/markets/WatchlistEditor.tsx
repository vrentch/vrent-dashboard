  ...
  return (
    <Sheet open={open} onClose={onClose} title="Watchlists">
      <div className="space-y-5">
        {/* Lists */}
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-2">Your lists</h3>
          <div className="flex flex-wrap gap-2">
            {prefs.watchlists.map((w) => {
              ...
              return (
                ...
                >
                  {w.name} <span className={on ? "text-white/70" : "text-slate-400"}>{w.symbols.length}</span>
                </button>
              );
            })}
            <button
              onClick={() => createWatchlist(`List ${prefs.watchlists.length + 1}`)}
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium bg-brand-50 text-brand-700 border border-brand-200 active:scale-95"
            >
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 px-3 py-2.5 rounded-xl bg-slate-100 border border-slate-200 text-sm font-semibold text-slate-900 outline-none focus:border-brand-500 focus:bg-white"
            aria-label="List name"
          />
          {prefs.watchlists.length > 1 && (
            <button
              onClick={() => deleteWatchlist(active.id)}
              className="grid place-items-center w-10 h-10 rounded-xl bg-rose-50 text-rose-600 active:scale-95"
        ...
        <div className="flex items-center gap-2 px-3 rounded-xl bg-slate-100 border border-slate-200 focus-within:border-brand-500 focus-within:bg-white transition">
          ...
          />
          <button onClick={() => add(input)} className="grid place-items-center w-8 h-8 rounded-lg bg-slate-900 text-white active:scale-95" aria-label="Add">
            <Plus size={18} />
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {active.symbols.map((s) => (
            <span key={s} className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-sm font-medium text-slate-900">
              {s}
              <button
          ...
          {!active.symbols.length && <p className="text-sm text-slate-400">No symbols yet — add some.</p>}
        </div>
