  ...
  return (
    <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4">
      {statuses.map((s) => (
        <div
          key={s.key}
          className="shrink-0 flex items-center gap-2 pl-2.5 pr-3 py-2 rounded-xl bg-white border border-slate-200/70 card-shadow"
        >
          <span className="text-base leading-none">{s.flag}</span>
          <div className="leading-tight">
            <div className="flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${s.open ? "bg-emerald-500" : "bg-slate-300"}`} />
